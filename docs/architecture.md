...
- **制品接受固定访问域。** 只读校验不持久化；上传、URL 和 GitHub 的确认安装接受精确摘要声明的五个访问域，
...
- **访问域不跨调用。** `network`、`models`、`accounts`、`requests`、`public_endpoints` 只开放对应资源域；
...
- OpenAI 与 xAI 用户身份选择由 PostgreSQL 保存，Core 在 `FrozenAccountScope` 中按 Key 整体覆盖通用选择，
  ...
  Desktop 完整制品元组原子更新。UA 发布列表由 OpenAI Provider 获取、解析并通过 Provider 目录缓存保存，
  Host 调度定期刷新，管理端可触发刷新；模型请求只读取已准备的目录。
  目录选择保存客户端、环境和完整 UA 快照，自动跟随只采用同组合的新条目，固定和自定义配置不被刷新覆盖。
  Provider 统一解析配套请求头并原样传递完整 UA；旧预设配置继续按原规则解析。
